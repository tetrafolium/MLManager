...
public ExtractFileInBackground(final Context context,
	...
	if (status) {
		...
		.showSnackbar(activity,
		              String.format(context.getResources().getString(
						    R.string.dialog_saved_description),
		                            appInfo.getName(),
		                            UtilsApp.getAPKFilename(appInfo)),
		              context.getResources().getString(R.string.button_undo),
		              UtilsApp.getOutputFilename(appInfo), 1)
		.show();
	} else {
		UtilsDialog.showTitleContent(
			...
			context.getResources().getString(
				R.string.dialog_extract_fail_description));
	}
}
}
