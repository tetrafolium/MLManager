...
protected void onCreate(final Bundle savedInstanceState) {
	...
	drawer = UtilsUI.setNavigationDrawer(
		(Activity)context, context, toolbar, appAdapter, appSystemAdapter,
...
protected Void doInBackground(final Void... params) {
	...
	default:
		...
		Collections.sort(packages, new Comparator<PackageInfo>() {
					...
					public int compare(final PackageInfo p1, final PackageInfo p2) {
					        ...
					        .toLowerCase()
					        .compareTo(
							packageManager.getApplicationLabel(p2.applicationInfo)
							.toString()
							.toLowerCase());
					}
				});
	...
	case "3":
		...
		Collections.sort(packages, new Comparator<PackageInfo>() {
					@Override
					public int compare(final PackageInfo p1, final PackageInfo p2) {
					        return Long.toString(p2.firstInstallTime)
	...
	case "4":
		...
		Collections.sort(packages, new Comparator<PackageInfo>() {
					@Override
					public int compare(final PackageInfo p1, final PackageInfo p2) {
					        return Long.toString(p2.lastUpdateTime)
		...
		if (!(packageManager.getApplicationLabel(packageInfo.applicationInfo)
			...
			if ((packageInfo.applicationInfo.flags &
				...
				try {
					...
					// TODO Workaround to avoid FC on some devices (OutOfMemoryError).
					// Drawable should be cached before.
					AppInfo tempApp = new AppInfo(
						packageManager
						.getApplicationLabel(packageInfo.applicationInfo)
						.toString(),
...
protected void onPostExecute(final Void aVoid) {
	...
	appFavoriteAdapter =
		new AppAdapter(getFavoriteList(appList, appSystemList), context);
...
private List<AppInfo> getFavoriteList(final List<AppInfo> appList,
                                      final List<AppInfo> appSystemList) {
	List<AppInfo> res = new ArrayList<>();

	for (AppInfo app : appList) {
		if (UtilsApp.isAppFavorite(app.getAPK(),
		                           appPreferences.getFavoriteApps())) {
			res.add(app);
		}
	}
...
public boolean onQueryTextChange(final String search) {
	if (search.isEmpty()) {
		...
		((AppAdapter)recyclerView.getAdapter())
		.getFilter()
	...
	case MY_PERMISSIONS_REQUEST_WRITE_READ: {
		if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
			UtilsDialog.showTitleContent(
				context, getResources().getString(R.string.dialog_permissions),
