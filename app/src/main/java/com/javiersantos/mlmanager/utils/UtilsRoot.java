...
public class UtilsRoot {
  ...
  public static boolean isRooted() {
    ...
    if (rootStatus == ROOT_STATUS_NOT_CHECKED) {
      isRooted = isRootByBuildTag() || isRootedByFileSU() ||
                 isRootedByExecutingCommand();
      MLManagerApplication.getAppPreferences().setRootStatus(
          isRooted ? ROOT_STATUS_ROOTED : ROOT_STATUS_NOT_ROOTED);
  ...
  public static boolean hideWithRootPermission(final String apk,
  ...
  public static boolean uninstallWithRootPermission(final String source) {
    boolean status = false;
    try {
      String[] command_write =
          new String[] {"su", "-c", "mount -o rw,remount /system\n"};
      String[] command_delete = new String[] {"su", "-c",
                                              "rm -r "
                                                  + "/" + source + "\n"};
      String[] command_read =
          new String[] {"su", "-c", "mount -o ro,remount /system\n"};
